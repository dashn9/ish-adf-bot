...
class NetworkRunner:
    ...
    def release_proxies(self):
        ...
        if self.identity.proxy_release_url:
            ...
            try:
                print(  # add to config,
                    main_requests.request(url=self.identity.proxy_release_url, method="GET", proxies=self.proxy).json())
            except:
                print(f"Bot Process Id {self.bot_process_id} <:::> An error occurred, might have failed to release")
    ...
    def request_interceptor(self, request: request.Request):
        ...
        def network_through_proxy(generate_empty_response_on_fail=True, retries=0):
            ...
            try:
                response = self.proxy_requests_session.request(url=request.url, verify=False, headers=request.headers,
                                                               allow_redirects=False, method=request.method,
                                                               data=request.body)
