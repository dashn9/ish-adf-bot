...
class BrowserInterface:
    ...
    async def open_url_on_browser(
        self, url: str, url_name: str, force_browser_diversion=False
    ):
        """
        ...
        if self.web_browser_driver:
            if url_name in self.opened_browser_urls:
                print(
                    url_name
                    + " is identifying a url already, use another url name or resolve code to select more unique name"
                )
                return False
            else:
                if not self.opened_browser_urls:  # If first Url To Browser,
                    ...
                    if url_id:
                        ...
                        print(
                            f"Bot Process Id {self.bot_process_id} <:::> Unable To Open Url: "
                            + url
                            + ", With Name: "
                            + url_name
                        )
            ...
            raise NameError(
                "No Available Chrome Browser, Either Set One Up by Using The "
    ...
    async def get_browser_window_body_size(self):
        return dict(
            width=await self.active_tab.evaluate(
                "document.body.getBoundingClientRect().width", await_promise=True
            ),
    ...
    async def get_browser_inner_size(self):
        return dict(
            width=await self.active_tab.evaluate(
                "window.innerWidth", await_promise=True
            ),
            height=await self.active_tab.evaluate(
                "window.innerHeight", await_promise=True
            ),
        )
    ...
    async def get_element_location_screen_offset(self, html_web_element: WebElement):
        ...
        web_element_x_offset = (
            web_element_location_dimensions.x + document_offset_from_screen["x"]
        )
        web_element_y_offset = (
            web_element_location_dimensions.y + document_offset_from_screen["y"]
        )

        browser_window_rect_bottom = (
            browser_window_rect.height + browser_window_rect.top
        )
        web_element_bottom = (
            web_element_y_offset + web_element_location_dimensions.height
        )
        return {
            "html_web_element": {
                "x_offset": web_element_x_offset,
                "y_offset": web_element_y_offset,
                "width": web_element_location_dimensions.width,
                "height": web_element_location_dimensions.height,
                "bottom": web_element_bottom,
            },
            "browser_window_rect": (
                browser_window_rect.left,
                browser_window_rect.top,
                browser_window_rect_bottom,
            ),
        }
    ...
    async def get_document_offset_from_screen(self):
        ...
        return dict(
            y=browser_window_rect.top
            + max((browser_window_rect.height - browser_inner_size["height"]), 0),
            x=browser_window_rect.left
            + max((browser_window_rect.width - browser_inner_size["width"]), 0),
            width=browser_inner_size["width"],
            height=browser_inner_size["height"],
        )
    ...
    async def get_window_document_offsets(self):
        ...
        return {
            "y_offset": (
                await self.active_tab.evaluate("window.pageYOffset", await_promise=True)
            )
            or 0,
            "x_offset": (
                await self.active_tab.evaluate("window.pageXOffset", await_promise=True)
            )
            or 0,
        }
    ...
    async def get_window_document_bounds(self):
        ...
        return dict(
            **offsets, **bounds, bottom=bounds.get("height") + offsets.get("y_offset")
        )
    ...
    async def activate_mobile(self, target_tab: Tab):
        await devtools_primary.activate_mobile(
            target_tab,
            {
                "width": self.identity.screen_resolution.get("logical_width"),
                "height": self.identity.screen_resolution.get("logical_height"),
                "device_scale_factor": self.identity.screen_resolution.get(
                    "density_pixel_ratio", 1
                ),
                "screen_orientation": {"type": "portrait_primary", "angle": 0},
                "mobile": True,
            },
        )
    ...
    async def quit_browser_after_max_alive(
        self, sleep_time=bot_constants.BOT_MIN_ALIVE_TIME
    ):
        await asyncio.sleep(sleep_time)
        if hasattr(self, "web_browser_driver"):
            if (
                time.time() - self.time_activated
                > bot_constants.BOT_MAX_ALIVE_TIME + random.uniform(-6.5, 6.5)
            ):
                logger.info(
                    f"*** Could Not Perform Activity Within Set Time, Exiting Session... ***"
                )
                try:
    ...
    async def open_web_browser(self):
        ...
        if (
            random.random() >= browser_constants.MAXIMUM_WINDOW_PROBABILITY
            and self.identity.device_type == "computer"
        ):
        ...
        if self.browser_to_use_id == browser_constants.CHROME_ID:
            ...
            browser_config.add_extension(
                f'{bot_constants.FULL_DIRECTORY_PATH+browser_constants.CHROME_EXTENSIONS_LOCATION+"/browser_spoofer.crx"}'
            )
            browser_config.add_extension(
                f'{bot_constants.FULL_DIRECTORY_PATH+browser_constants.CHROME_EXTENSIONS_LOCATION+"/browser_network.crx"}'
            )
            ...
            self.web_browser_driver = await uc.start(config=browser_config)
            ...
            if open_browser_in_full_screen:
                ...
                if (
                    random.random() <= browser_constants.FULLSCREEN_PROBABILITY
                    and not config.CONTAINERIZED
                ):
                    logger.info("{{{ Setting Window Size To Fullscreen... }}}")
                    await self.active_tab.set_window_state(
                        0, 0, *window_size, state="fullscreen"
                    )
                elif config.CONTAINERIZED:
                    logger.info(
                        "{{{ Setting Window Size To Maximized: Container Method... }}}"
                    )
                    await self.active_tab.set_window_state(
                        0, 0, *window_size, state="fullscreen"
                    )
                    await self.active_tab.set_window_state(
                        0, 0, *window_size, state="maximized"
                    )
                else:
        ...
        await devtools_primary.listen_to_tab_creation(
            self.web_browser_driver.connection, self._handle_new_tab_creation
        )
        ...
        logger.info(
            "{{{ Executing JS: Dispatching Event With Identity Spoof Data... }}}"
        )
        ...
        identitySpoofData = json.dumps(
            await self.identity.fetch_identity_data_for_extension()
        )
    ...
    async def _handle_new_tab_creation(self, new_tab):
        ...
        for tab in self.web_browser_driver.tabs:
            ...
            if target_id not in self.preliminary_activated_tab_ids:
                ...
                logger.info(
                    "{{{ Subscribing To Page Events For Tab: %s }}}" % (target_id)
                )
    ...
    async def _subscribe_tab_to_lifecycle_events(self, tab: Tab):
        ...
        await devtools_primary.listen_to_page_lifecycle(
            tab, self._handle_page_event_for_tab
        )

    async def _handle_page_event_for_tab(self, event):
        logger.info(
            f"000 Received Lifecycle Event: {event.name} For Tab: {event.frame_id} 000"
        )
        frame_id = str(event.frame_id)
        self.pages_lifecycle_events[frame_id] = self.pages_lifecycle_events.get(
            frame_id, []
        )
    ...
    async def wait_for_tab_load(self, tab: Tab | None = None, timeout=20):
        ...
        while time.time() - start_time < timeout:
            ...
            if getattr(
                self.pages_lifecycle_events.get(target_id, [""])[-1], "name", None
            ) in [
                "networkAlmostIdle",
    ...
    async def preliminary_tab_activation(self, target_tab: Tab):
        ...
        if self.identity.device_type == "smartphone":
            logger.info(
                "{{{ Activating Mobile On Browser For Device: %s... }}}"
                % (self.identity.hardware)
            )
            await self.activate_mobile(target_tab)
        await target_tab.sleep(0.5)
        logger.info(
            "{{{ Setting Hardware Concurrency: %d... }}}"
            % (self.identity.hardware_concurrency)
        )
        await devtools_primary.set_hardware_concurrency(
            target_tab, self.identity.hardware_concurrency
        )
        logger.info(
            "{{{ Setting Up Timezone: %s... }}}" % (self.identity.timezone.get("id"))
        )
        await devtools_primary.set_timezone(
            target_tab, self.identity.timezone.get("id")
        )
        ...
        await devtools_primary.change_user_agent(
            ...
            self.identity.languages,
            self.identity.browser_name,
            self.identity.device_type == "smartphone",
