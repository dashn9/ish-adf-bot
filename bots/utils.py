...
def generate_uniform_numbers_to_specific_range_and_sum(value_range: tuple = (0, 1), total_values_length=10, sum=5):
    if value_range[0] < 0 or value_range[0] >= value_range[1]:
        ...
        warnings.warn(f"Maximum Range Of {value_range[1]} Has Been Overwritten With {sum / total_values_length}, \
    ...
    if overhead != 0:
        warnings.warn("Unable To Generate Accurate Sum With Given Parameters")
