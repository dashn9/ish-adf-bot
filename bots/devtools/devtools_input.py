...
class Keyboard:
    ...
    def down(self, key, options={'text': None}):
        description = self.key_description_for_string(key)

        auto_repeat = False
        if description['code'] in self.pressed_keys:
            auto_repeat = True
        if self.down_key_number_of_consecutive_runs == 1:
            time.sleep(random.uniform(0.5, 0.52))

        self.pressed_keys.add(description['code'])
        self.modifiers |= self.modifier_bit(description['key'])

        text = description['text'] if 'text' in options else options.text
        type = 'keyDown' if text else 'rawKeyDown'
        is_keypad = False
        if 'location' in description and description['location'] == 3:
            is_keypad = True
        self.webdriver.execute_cdp_cmd('Input.dispatchKeyEvent', dict(
            type=type,
    ...
    def down_persistent(self, key, options={'text': None}):
...
class Mouse:
    ...
    def move(self, x, y, options={'steps': 1}):
        steps = options['steps']
        from_x = self.x, from_y = self.y
        self.x = x    
        self.y = y    
        for i in range(steps):
            self.webdriver.execute_cdp_cmd('Input.dispatchMouseEvent', dict(
                type= 'mouseMoved',
                button= self.button,
                x= from_x + (self.x - from_x) * (i / steps),
                y= from_y + (self.y - from_y) * (i / steps),
                modifiers= self.keyboard.modifers
            ))
...
class Touchscreen:
    ...
    def simulate_human_touch_movement_with_mouse(self, start_point=(0, 0), end_point=(0, 0), duration=3):
        ...
        coords_distance = utils.find_points_distance_on_2d_cartesian_plane(start_point, end_point)
        ...
        human_curve = HumanCurve(start_point, end_point)
        ...
        points = human_curve.generateCurve(offsetBoundaryX=40, offsetBoundaryY=0,
                                           leftBoundary=left_boundary, rightBoundary=right_boundary + 1,
