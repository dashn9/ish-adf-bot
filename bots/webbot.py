...
class WebBot(BrowserInterface, SmartHumanReader,
             NetworkRunner):  # A powerful WebBot designed to visit and perform activities on given url/s
    ...
    def __init__(self, identity=None,
                 ...
                 bot_process_id=None,
                 no_of_clicks=0, use_proxy=False
                 ):
        ...
        if not identity:
            self.identity = Identity(
                ...
                referer="https://l.facebook.com", reading_speed=900, timezone=["Etc/GMT", 0, "AM Coordinated Time"],
                mouse_delta_y=50)
        else:
            self.identity = identity

        network_runner = NetworkRunner.__init__(self, bot_process_id, identity, use_proxy)
        browser_interface = BrowserInterface.__init__(
            self, browser_to_use_id=browser_to_use_id, driver_path=driver_path, network_runner=network_runner,
            bot_process_id=bot_process_id, timezone_id=identity.timezone[0], device_type=identity.device_type,
            has_touch=identity.has_touch, has_mouse=identity.has_mouse, languages=identity.languages,
            user_agent=identity.user_agent, hardware=identity.hardware, platform=identity.platform,
            screen_width=identity.screen_width, screen_height=identity.screen_height,
            device_pixel_ratio=identity.screen_resolution[4], cookies=identity.cookies, identity_id=identity.id,
            cookies_update_callback=identity.update_cookies)
        SmartHumanReader.__init__(self, browser_interface, reading_speed=identity.reading_speed,
