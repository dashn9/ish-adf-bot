...
class HumanBehaviourReveries:
    ...
    def __init__(self, browser_instance: BrowserInterface, human_movements: HumanMovements):
        self.browser_instance = browser_instance
        self.bot_process_id = self.browser_instance.bot_process_id
    ...
    def move_mouse_to_fool_exit_point(self):
        ...
        if HumanBehaviourReveries.active_on_mouse_movement.value < 0:
            ...
            time.sleep(0.5)
            HumanBehaviourReveries.active_on_mouse_movement.value = -self.bot_process_id if self.bot_process_id != 0 \
                else -500
            return True
        return False
    ...
    def move_mouse_to_random_area_on_screen(self):
        if HumanBehaviourReveries.active_on_mouse_movement.value < 0:
            HumanBehaviourReveries.active_on_mouse_movement.value = self.bot_process_id
            self.browser_instance.bring_window_to_front()
            self.human_movements.\
                simulate_human_mouse_move_behavior_to_area(0, 0,
                                                           ...
                                                           max_overshoot=35, probability_of_overshoot=round(
                                                                                random.random(), 2))
            time.sleep(0.5)
            HumanBehaviourReveries.active_on_mouse_movement.value = -self.bot_process_id if self.bot_process_id != 0 \
                else -500
    ...
    def open_link_in_elements(self, elements):
        ...
        if HumanBehaviourReveries.active_on_mouse_movement.value < 0 and len(links_to_follow) >= 1:
            HumanBehaviourReveries.active_on_mouse_movement.value = self.bot_process_id
            print(f"Bot Process Id {self.bot_process_id} <:::> Attempting To Open A Link In Related Articles")
            self.browser_instance.bring_window_to_front()
            link_to_follow = links_to_follow[random.randint(0, len(links_to_follow) - 1)]
            self.human_movements.move_pointing_device_to_element(link_to_follow)
            time.sleep(random.uniform(0.2, 0.8))
            if self.browser_instance.has_touch:
                time.sleep(random.uniform(0.3, 0.5))
                try:
                    element_location_and_dimensions = self.browser_instance.get_element_location_window_offset(
                        link_to_follow)
                    # Do click continually until page remained unchanged after click
                    self.human_movements.touch.tap(element_location_and_dimensions["x_offset"] +
